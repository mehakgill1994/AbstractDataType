...
//-----------------------------------------------
/*
* This is a free function to print the eigen values.
  parameters:
		 vector<double> : a vector containing real
					and imaginary part of eigenvalue
			int			: an integer to tell which
					root to print i.e. 1 or 2.
*/
//-----------------------------------------------
void printEigenvalues(std::vector<double> v, int n) {
	...
	if (v.size() == 1) {
		...
		if (v[1] == (int)v[1])
			cout << setprecision(0);
		cout << showpos << v[1] << "i" << noshowpos << endl;
	}
}
